...
def insert_wrk_tournaments_win():
    tournaments_win_data = []
    ...
    for file in os.listdir(output_directory):
        with open(f"{output_directory}/{file}", encoding="utf-8") as f:
            ...
            tournament_id = tournament['id']
            tournament_name = clean_text(tournament['name'])
            name = clean_text(tournament['players'][0]['name'])
            placing = tournament['players'][0]['placing']
            victories = tournament['players'][0]['victories']
            losses = tournament['players'][0]['losses']
            draws = tournament['players'][0]['draws']
            winrates = tournament['players'][0]['winrates']
            deck = tournament['players'][0]['deck']

            tournaments_win_data.append((
                tournament_id, tournament_name, name, placing, victories, losses, draws, winrates, deck
            ))

    with psycopg.connect(get_connection_string()) as conn:
        with conn.cursor() as cur:
            cur.executemany("INSERT INTO public.wrk_tournaments_winVALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)", tournaments_win_data)
...
print("insert raw tournament win data")
insert_wrk_tournaments_win()